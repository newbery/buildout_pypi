...
def extend_parser(parser):
    ...
    if sys.version_info <= req_python:
        print('ERROR: Requires Python %d.%d or greater.' % req_python)
        sys.exit(101)

    INCLUDE_PY = distutils.sysconfig.get_config_vars()['INCLUDEPY']
    if not os.path.exists('%s/Python.h' % INCLUDE_PY):
        print('ERROR: %s' % sys.exc_info()[1])
        print('ERROR: This buildout requires the Python dev files.')
        print('You probably are using a non-development install of Python.')
        sys.exit(101)

    #parser.set_defaults(use_distribute=True, never_download=True, clear=True)
    parser.set_defaults(use_distribute=True, clear=True)


def adjust_options(options, args):
    #options.search_dirs.append("./virtualenv/downloads")

    # Create virtualenv in subdirectory
    this_dir = os.path.dirname(os.path.abspath(__file__))
    args.append(os.path.join(this_dir, 'virtualenv'))


def after_install(options, home_dir):
    if os.path.exists('bootstrap.py'):
        logger.notify('Running bootstrap.py')
        subprocess.call([join(home_dir, 'bin', 'python'), 'bootstrap.py'])
